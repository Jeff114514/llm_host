#!/usr/bin/env python3
"""使用 Python 启动 sglang OpenAI Server."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="启动 sglang Python 进程")
    parser.add_argument(
        "--config-file",
    ...
    parser.add_argument(
        "--stop",
        action="store_true",
        help="仅停止当前正在运行的 sglang 进程",
    )
    parser.add_argument(
        "--restart",
        action="store_true",
        help="先停止再重新启动 sglang",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="等待 sglang /health 就绪的超时时间（秒）",
    )
...
def main():
    ...
    if args.stop:
        if manager.is_running():
            ...
            print("[sglang] 已停止正在运行的进程")  # noqa: T201
        else:
            print("[sglang] 当前没有运行中的进程")  # noqa: T201
        return
    ...
    print(f"[sglang] 进程 PID: {pid}, ready={ready}")  # noqa: T201

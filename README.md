...
**vLLM Proxy** 是一个基于 **FastAPI + vLLM + Nginx** 的本地 LLM 部署与代理方案，提供：

- 基于文件的 **API Key 认证与 admin 权限管理**
- **QPS / 并发 / Token** 三级限流
- **Prometheus + Grafana** 指标与可视化
- **结构化 JSON 日志** 与自动日志轮转/清理
- 对 vLLM OpenAI 兼容接口的 **代理与多 LoRA 管理**

整体架构：

```text
Client → Nginx (反向代理) → FastAPI (认证 / 限流 / 监控) → vLLM (OpenAI API 兼容)
```
...
- **vLLM**：LLM 推理服务，提供 OpenAI 兼容 API
